...
class ReceiveAttendeController extends GetxController {
  ...
  Future<void> setInitialData() async {
    if (args != null) {
      ...
      if (args.containsKey('userType') && args['userType'] != null) {
        userType = args['userType'];
      } else {
        userType = UserTypeEnum.loggedIn;
      }
      if (args.containsKey('receptionistGuest') &&
          args['receptionistGuest'] != null) {
        receptionistGuest = args['receptionistGuest'];
      }
    }
  }

  Future<void> receiveAttendeeAsGuest() async {
    try {
  ...
  Future<void> receiveAttendee() async {
    try {
      ...
      var identifier = faceRecognitionResponse!.result!.user!.id ?? '';
      var clientId = faceRecognitionResponse!.result!.user!.clientId;
      var attendeeId = identifier.replaceAll('${clientId}_', '');
      ...
      var response = await _eventRepository.receiveAttende(
        id: eventData.id ?? '',
        data: data,
        image: image,
      );
  ...
  Future<void> recognizeFace() async {
    ...
    faceRecognitionResponse = await _faceRecognitionRepository.recognizeFace(
      ...
      subEventId:
          userType == UserTypeEnum.guest ? eventData.identifikaEventId : null,
    );
    ...
    if (faceRecognitionResponse!.result!.verified!) {
      if (userType == UserTypeEnum.loggedIn) {
        await receiveAttendee();
      } else {
        await receiveAttendeeAsGuest();
      }
    } else {
      CustomDialogWidget.closeLoading();
